...
上下文压缩解决的是“历史太长时如何减负”，但它只是上下文工程（Context Engineering）的一部分。

上下文工程关注的是：在每次模型调用前，如何用稳定、分层、可缓存、可验证的信息组合出“此刻完成任务所需的最小可执行上下文”。它不仅决定 token 成本，也决定模型能否稳定遵守约束、正确选择工具和延续任务。

### 1. Claude Code 的上下文组装模型

源码中的 `src/utils/systemPrompt.ts` 和 `src/utils/queryContext.ts` 表明，系统提示词并不是单块字符串，而是按优先级组合：

```text
系统级稳定指令
  → 协调者或专用 Agent 指令
  → 项目规则与长期记忆
  → 工具定义、技能（Skills）、模型上下文协议（MCP）说明
  → 当前计划、权限模式、工作目录等运行时状态
  → 用户本轮输入、近期消息、工具结果
```

其中越稳定、越高频复用的内容越应靠前；越动态、越短期的内容越应靠后。这样既让模型优先看到不应违反的契约，也有利于提示词缓存（Prompt Cache）复用稳定前缀。

### 2. 上下文工程的四个动作

| 动作 | 要解决的问题 | 推荐做法 |
|---|---|---|
| 选择 | 什么信息应该进入本轮 | 只加载当前任务、目录、工具和阶段真正需要的资料 |
| 排序 | 什么信息必须优先稳定 | 系统契约、权限边界、项目规则在前；动态结果在后 |
| 压缩 | 什么信息不必保留原文 | 清理旧工具输出，用会话记忆或摘要替代历史 |
| 重建 | 压缩或恢复后如何继续执行 | 重新注入计划、附件、工具、技能、MCP、钩子和权限状态 |

压缩不是终点：压缩后若不重建执行环境，模型虽然有一段摘要，却不知道自己还能用什么工具、处于何种权限模式，也不知道哪些计划已被批准。

### 3. 提示词缓存会反向塑造架构

`src/services/api/promptCacheBreakDetection.ts` 会跟踪系统提示、工具集合和缓存控制字段的变化。它反映出一个工程事实：频繁变动的规则、工具模式或长文本附件会破坏缓存前缀，使每一轮都重新支付大量输入 token。

因此应当：

- 将稳定系统规则、工具参数模式（schema）和项目契约保持固定顺序。
- 将按需信息设计为附件、检索结果或技能，而不是全部塞进常驻提示。
- 避免每轮重写等价的动态指令；用稳定标识引用持久化状态。
- 在工具、MCP 或权限状态变化时，明确接受缓存失效，而不是隐性地让成本和延迟失控。

### 4. 上下文工程的落地路径

1. 先定义稳定的系统契约与项目规则，并保持其顺序稳定。
2. 将计划、待办、权限、工作目录、验证状态外化为结构化运行时状态。
3. 对文件、搜索、日志等高体积信息采用按需读取与输出预算。
4. 通过会话记忆和分层压缩管理长历史。
5. 在压缩、恢复、切换 Agent 后重建最小可执行上下文。
6. 监控输入 token、缓存读取量、工具选择失败率和压缩后返工率，持续调整上下文组成。

上下文工程的底线是：

> 任何一轮模型调用都不应依赖“模型碰巧还记得”；任务目标、硬约束、当前状态和可执行能力必须以恰当的形式重新可见。
